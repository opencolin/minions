...
**Concrete instantiation:** [IronClaw](/approaches/#openclaw) — a Rust agent runtime that runs inside encrypted TEEs, with isolated WebAssembly containers per tool, credentials in an encrypted vault that intercepts outbound connections against policy, and sub-30-second attestation. The **Confidential GPU Marketplace** locks out the host OS, GPU operators, and NEAR AI itself from inference workloads. Worth noting: IronClaw is built by NEAR AI but is positioned as the OS layer of the broader [OpenClaw ecosystem](/approaches/#openclaw) — the same lineage that produced [crabbox](/approaches/#crabbox), [clawpatch](/approaches/#clawpatch), and [ClawSweeper](/approaches/#clawsweeper).
...
> See: [Approaches § Stripe Minions](/approaches/#stripe-minions)
...
> See: [Approaches § GStack](/approaches/#gstack) + [GBrain](/approaches/#gbrain)

Pattern: one-engineer, opinionated SKILL.md skill pack + parallel worktree execution + persistent markdown brain. [Garry Tan's](/who-is-who/#garry-tan) stated workflow runs 10–15 parallel Claude Code sessions inside [Conductor](/infrastructure/#autonomous-coding-agents) on a single Mac with gstack as the engine and gbrain as the memory companion.
...
> See: [Harness Engineering](/harness-engineering/)
...
**Philosophical alignment:** Almost pure Ng — process / evals / error analysis as the load-bearing layer, infrastructure interchangeable. The "thin harness, fat skills" ethos that recurs in [GBrain](/approaches/#gbrain) and [AgentHub](/approaches/#agenthub) is the direct lineage.

### The Steinberger School

> See: [Approaches § The Steinberger School](/approaches/#the-steinberger-school)

Pattern: token-unbounded automation. [Peter Steinberger's](/who-is-who/#peter-steinberger-steipete) reported workflow runs ~100 Codex agents continuously, ~$1.3M/month inference spend, ~3 engineers. Every project event — PR review, commit security scan, issue triage, meeting transcript, performance regression — has an agent listening. Supporting toolchain: [Crabbox](/approaches/#crabbox), [Clawpatch](/approaches/#clawpatch), [ClawSweeper](/approaches/#clawsweeper), Discrawl, CodexBar, fs-safe, peekaboo.
...
For the operational schools, see the per-school links inline above. For the underlying methodology, see [Harness Engineering](/harness-engineering/).

...
Hermes started as a row in the OpenClaw ecosystem table but is now its own pillar. It's a self-improving personal agent: it runs persistently with memory across sessions, distills reusable skills from completed work, and curates its own skill library on a recurring cycle. Garry Tan's [GBrain](/approaches/#gbrain) project explicitly names Hermes Agent as one of its two reference host platforms (alongside [OpenClaw](/approaches/#openclaw)).
...
- **Six terminal backends**: local, Docker, SSH, [Daytona](/sandboxes/#purpose-built-agent-sandboxes), Singularity, [Modal](/sandboxes/#purpose-built-agent-sandboxes) — so the same agent can execute code locally for cheap or in a managed sandbox for safety
...
- **Modal / Daytona terminal backends** show how the personal-agent space is composing with the sandbox vendor landscape we cover in [Sandboxes](/sandboxes/)
...
- You need a coding-specific harness → [Claude Code](/approaches/#terminal-coding-clis) + [GStack](/approaches/#gstack), [OpenHands](/approaches/#openhands), or [Vercel Open Agents](/approaches/#vercel-open-agents)
